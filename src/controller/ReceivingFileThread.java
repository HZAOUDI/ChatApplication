...
public class ReceivingFileThread extends Thread {
    ...
    public void sendToServer(String line) {
        try {
            ...
            JOptionPane.showMessageDialog(null, "Serveur arrêté, Impossible d'envoyer un message!", "Erreur", JOptionPane.ERROR_MESSAGE);
        } catch (IOException ex) {
    ...
    public void run() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(frameToDisplayDialog, "Fichier enregistré dans\n"+myDownloadFolder + "\\" + fileName, "Info", JOptionPane.INFORMATION_MESSAGE);
        } catch (IOException ex) {
            Logger.getLogger(ReceivingFileThread.class.getName()).log(Level.SEVERE, null, ex);
            System.out.println("This socket for receiving file has benn close!");
        } finally {
            try {
